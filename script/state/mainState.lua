    ...
    local score = {
        ...
        components = {
            ...
            ScriptComponent:new(nil, function ()
                    ...
                    local posText = Vector2D:new(engine:getWinWidth() // 2 - size_score, 10)
                    engine:renderText(posText, size_score , 80, tostring(mainState.score))

                    if player.gameover == false then
                        engine:setColor(0xff, 0xFF, 0x00, 0xFF)
                        engine:renderText(Vector2D:new(), 64, 64, "O2: " .. player.oxygen .. "%")
                    end
                    if player.gameover then
                        engine:setColor(0xff, 0x00, 0x00, 0xFF)
                        engine:renderText(Vector2D:new(), engine:getWinWidth(), 128, "GAME OVER")
                    end
                end
            ),
        }
    ...
    muteButton = {
        ...
        components = {
            ...
            ScriptComponent:new(nil, function ()
                ...
                engine:renderText(muteButton.entity:getPosition(), 32, 32, "MUTE")
            end)
        }
    }
    mainState.uis = {
